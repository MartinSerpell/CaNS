...
void Group_1(BYTE opcode)
{
	...
	switch(opcode) 
	{
		...
		case 0x5E: // LDX abs
			...
			if (address >= 0 && address < MEMORY_SIZE - 1)
			{
				IndexRegister = (WORD)Memory[address] << 8;
				IndexRegister += Memory[address + 1];
			}
			break;
		...
		case 0x50: // STOX abs
			...
			if (address >= 0 && address < MEMORY_SIZE)
			{
				Memory[address] = IndexRegister >> 8;
				Memory[address + 1] = IndexRegister;
			}
			break;
